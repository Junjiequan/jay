...
const openEmailBox = () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (emailBox.style.display === 'block') {
      notEmailBox && notTrigger && notNavContact ? close() : false;
    }
  });
  document.addEventListener('keydown', (e) =>
    e.key === 27 && emailBox.style.display === 'block' ? trigger.click() : false
  );
  dropdown.addEventListener('click', () => {
    dropdownIcon.style.transform === '' ? emailInputDropdown.show() : emailInputDropdown.hide();
  });
};
...
form.addEventListener('submit', (e) => {
  ...
  emailjs.sendForm('service_hn4x94s', 'FromPortfolio', form, '_vC5IRi7AoVWfPldq').then((result) => {
